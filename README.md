...
A session is finished when its turn ends with nothing left running in the background. When Claude ends a turn while agents it launched are still working, the session stays working and its line reads `project · working · 2 agents · host · age`; the finished cue comes with the turn that ends after the last agent is done, which is when the conclusions arrive. Claude Code's reminder that it is waiting for input, sent a minute after every turn, is not a needs-you cue: permission prompts, questions from agents and dialogs are.
...
- Jumping to a session switches tabs in Windows Terminal by tab title; in other tabbed terminals the window comes forward and the tab is yours to pick.
