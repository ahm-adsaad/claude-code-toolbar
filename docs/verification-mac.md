...
| 32 | Finished cue | Answer the prompt and let Claude finish: badge turns green with a single soft chime; click the item away from Clawd: the popover shows "project · finished · <host> · now"; close the popover and the badge disappears; leave the session idle for two minutes: no amber badge and no chime (Claude Code's idle reminder is ignored) | |
| 32a | Finished waits for agents | Ask Claude to launch a background agent that runs for a minute and to reply right away: when its reply ends the badge stays blue and the popover line reads "project · working · 1 agent · <host> · now"; the green badge and the chime come only after the agent is done and Claude has reported | |
...
| 34 | Session lines | With two Claude Code sessions running (one in Terminal, one in VS Code) click the item away from Clawd: the popover lists both as `project · state · host · age` with the right host names | |
...
| 40 | Energy | Leave the app idle for 10 minutes, then Activity Monitor → Energy: ClaudeToolbar's Energy Impact stays near 0 (single digits at most) and it is not listed under the battery menu's "Using Significant Energy"; note the "12 hr Power" figure before and after updating | |
| 41 | Menu bar still live | The countdown in the menu bar still changes at each new minute, hovering still waves Clawd, a needs-you session still blinks the badge, and switching light/dark mode repaints the text in the right colour straight away | |
| 42 | Pause while nobody looks | Lock the screen (Ctrl+Cmd+Q) for a minute and unlock: `~/Library/Logs/ClaudeToolbar/` shows "Pausing updates" then "Resuming updates" and the numbers are current at once; a session that finishes while locked still chimes | |
| 43 | Keychain read once | Over 10 minutes of normal use the log shows no repeated credential errors; signing in again in Claude Code (`/login`) is picked up within a refresh, because a rejected token makes the app read the Keychain again | |
